...
async def test_film_search(generate_movies, make_get_request):
    ...
    response = await make_get_request(f'films', params={'search': film_title})
...
async def test_pagination_page_size(generate_movies, make_get_request):
    ...
    PAGE_SIZE = 4
    response = await make_get_request(f'films', params={'page_size': PAGE_SIZE})
...
async def test_pagination_metadata_page_1(generate_movies, make_get_request):
    ...
    response = await make_get_request(f'films', params={
        'page': 1, 
        'page_size': 1, 
    })
