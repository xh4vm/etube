...
async def test_get_user_roles(
    ...
    assert {
        (user.get('login'), user.get('email'),) 
        for user in users_recv
    } == {
        ('cheburashka', 'chebu@rash.ka',)
    }
...
async def test_set_role(make_request, generate_roles, generate_users, pg_cursor):
    ...
    select_statement = f"SELECT id FROM {CONFIG.DB.SCHEMA_NAME}.user_roles WHERE role_id = '{role_id}' AND user_id = '{user_id}';"
    ...
    response = await make_request(
        method='post',
        target=f'auth/manager/user/role',
        json={
            'role_id': role_id,
            'user_id': user_id
        },
